...
from utils_noise import rollout as rollout_noise
...
def test_const_noise_adv(env, protag_policy, path_length=100, n_traj=5, render=False, std_noise=0.001):
    const_adv_policy = ConstantControlPolicy(
        env_spec=env.spec,
        is_protagonist=False,
        constant_val = 0.0
    ...
    sum_rewards = 0.0
    for _ in range(n_traj):
        path = rollout_noise(env, protag_policy, path_length, adv_agent=const_adv_policy, animated=render, test=True, std_noise=std_noise)
        sum_rewards += path['rewards'].sum()
        paths.append(path)
    avg_rewards = sum_rewards/n_traj
    return avg_rewards
