...
LOGDIR=/home/${USER}/tmpoutputs_no_adversary_${ENV}/
if [ ! -d ${LOGDIR} ]; then
...
do
    ...
    do
    	...
    	do
            	...
            	echo $PROCSTRING
            	qsub -N ${PROCSTRING} -q ${QUEUE} -l nodes=1:ppn=8 -l walltime=99:99:99:99 ${LOGSTRING} -v LOGDIR=${LOGDIR},ENV=${ENV},step_size=${step_size},gae_lambda=${gae_lambda},batch_size=${batch_size},PROCSTRING=${PROCSTRING},adv_fraction=${adv_fraction} yoda_rl_no_adversary_driver.sh
    	done
        done
    done
done
