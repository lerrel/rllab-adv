...
do
    ...
    do
    	...
    	do
            	PROCSTRING="$ENV-$step_size-$gae_lambda-$batch_size-$adv_fraction-$NITR-meantest"
            	echo $PROCSTRING
		...
		for ((i = 1; i <= $NEXP; i++)); do
    			echo $i
            		qsub -N ${PROCSTRING} -q ${QUEUE} -l nodes=1:ppn=4 -l walltime=99:99:99:99 ${LOGSTRING} -v SAVEDIR=${SAVEDIR},LOGDIR=${LOGDIR},ENV=${ENV},step_size=${step_size},gae_lambda=${gae_lambda},batch_size=${batch_size},PROCSTRING=${PROCSTRING},adv_fraction=${adv_fraction},NITR=${NITR} yoda_rl_adversary_single_driver.sh
		done
    	done
        done
    done
